...
**觸發條件**:MSCI 季度再平衡公告後 5 個交易日內,MSCI Taiwan 指數權重從當季變動 > +2pp 或單季上調至 > 30%,觸發「被動 ETF 必須增持台股」結構訊號(對位再平衡 + 反向警示)

**反向觸發**:MSCI Korea / Taiwan 任一權重單季下調 > 1.5pp,觸發「被動 ETF 必須減持」= **直接對位 2026/6 韓股崩盤真因**(本模板雙向)
...
- `/api/event/calendar` — MSCI 季度再平衡公告日(2/8/11 月)
- 需新增 `msci_rebalance_analyzer` 服務計算「權重變動 vs 觸發」(**atlas 端未暴露 = web fallback MSCI 官網**)

**新增日期**:2026-08-09 | **序號**:第 17 模板
**新增原因**:v6.58 研究發現**韓股 2026 崩盤真因之一 = Samsung/SK Hynix 權重飆升 → 被動 ETF 被迫減持 → 結構性賣壓**(對位 v6.58 §3.1 「MSCI 季度再平衡」觸發鏈)。MSCI Taiwan 4/30/2026 已達 24.8%(全 EM 最大權重),若再升,觸發條件成熟
...
- MSCI 季度再平衡公告日:2026/8 公告 = 11月生效前 5 個交易日為關鍵監測期
...
- **反向觸發 = 直接對位 2026 韓股崩盤真因**(Samsung/SK Hynix 權重飆升 → ETF 被迫減持 → 結構性賣壓)
