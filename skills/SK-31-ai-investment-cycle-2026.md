...
4. **TSMC 2025 announced $100B 半導體投資**(= US-TW 跨境 greenfield 約 1/3) — UNCTAD WIR 2026 Chapter I
...
6. **能源 / 勞動力 / 治理「三限制」**:HKS 案例 Singapore 2019→2022 數據中心 moratorium,delay 4–6 年,警示台灣 AI / data center 投資落地週期可能比報告預期長。對位 atlas 既有 trigger `taiwan_political_risk`(impact -0.9),但**缺一條 trigger 對位「AI capex surge + 三限制」這條雙向賽局**。
7. **2027~2028 訂單跳升點**:UNCTAD + HAI 數據 + HKS 案例綜合顯示,**AI 宣布 → 設備下單 6~18 個月,試產 / 量產 +12~+24 個月**。對位台股:2026 Q3~Q4 設備鏈首次訂單能見度,2027 全年~2028 Q1 月營收跳升。

## atlas 對位

| atlas 既有機制 | hit_rate / score | 與本週期對位強度 | 缺口 |
|----------|---------|----------|----------|
| `AI_capex_surge` detector(24 detectors 之一) | conf 0.95 | ✅ **強對位** — 已上線,2026-08-04 01:25 產出 score 0.7343 narrative chain | 缺「外部報告週期」(annual / semi-annual)cadence 重置 |
| `ai_supercycle_model` narrative(3 models 中 weight 最大 = 0.1639) | historical_hit_rate 0.625 / recent 0.625 implied | ✅ **強對位** — favored sectors 完全對位 HAI §2.7 設備鏈名單 | 缺「報告週期重置 hit_rate」機制 |
| `trigger-nvda-tsm.md`(13 templates) | 已上線 cron 5min | ✅ 中對位 — 對應 HAI §2.3,但缺時滯機制 | — |
| **未存在:`trigger-megaproject-2-quarter-lag.md`** | — | ❌ **缺** | B 階段落模板 #14 |
| **未存在:`trigger-equipment-capex-external-report-cycle.md`** | — | ❌ **缺** | B 階段落模板 #15 |
...
1. **立刻可看**:`narrative_get_chains` → 看 score 排名,前 3 條若全部偏多 AI 供應鏈 = 訊號偏多,可不追;若 score 波動 > 30% / 24h = 警戒。
...
4. **散戶不可做的事**:用「CAPE 處歷史高檔」當作「不看 AI 週期」的理由——AI 供應鏈是 mission「找信息差」的具體對位,CAPE 是 mission「防禦層」,兩層是 mission 內兩件事,**沒有誰取代誰**。

## 驗證方式

- **L3 端點實跑 ✅(2026-08-04 01:25:54Z)**:`macro_get_stress_index_current` 4.22 low;`narrative_get_chains` 5 chains(`AI_capex_surge` 0.7343 最高);`narrative_get_models` `ai_supercycle_model` hit_rate 0.625;`stock_get_quote` 2330(PE 30.19)、3680(last 427);`system_get_health` 真實上線(2026-08-04 跑通)。
- **對位 ATLAS_METHODOLOGY.md v1.0 ✅**:引用七時期 + 三態向下相容 + RiskLevel + 七維錢潮雷達 + 策略三分類(Defensive 觀望 / Aggressive 押 AI 供應鏈 / Tactical 事件套利)。
- **格式 ✅**:6 段標準段名 + frontmatter 核心欄位齊全(逐日由 `make check-skill-pages` 驗證)。
...
- `_consult-index.md` §Q1 T3-A248 / `12-ext-research/2026-un-harvard-ai-investment/README.md` / `_method.md` 鐵律六 / `docs/ATLAS_METHODOLOGY.md` v1.0 §一 §二
